...
const NOTES = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      'The second is about motion. Is anything moving that does not need to? Entrance animations, scroll-linked tricks, decorative hover flourishes. They are easy to add and hard to subtract once they are in. If a motion does not change how the user understands the interface, I take it out before anyone else sees it.',
      'The third is about touch screens and hover. On a phone there is no cursor — only taps. If a control changes color on :hover but not on :active, the UI can feel sticky or wrong after you lift your finger. Before I ship, I ask whether hover is doing real work. Lately that means gating hover styles behind something like lg:hover: on links and buttons, and sometimes a scoped CSS guard on mobile so color and transform changes only apply where hover actually exists. Clicks and active states stay; fake hover goes.',
      'The fourth is about silence. Have I left enough space around the content for it to breathe, or did I fill every gap because empty space made me nervous? The answer is usually that I filled too much. Most of my best edits late at night are deletions.',
      'The fifth is about copy. Is the smallest text on the page still considered? Microcopy is where you can tell whether someone was paying attention. Labels, empty states, error messages, the tag under a card. Those are the lines that decide whether the product reads as thoughtful or just well-designed.',
      ...
      'None of these questions are novel — they are the obvious things, written down so I actually run them. Doing that has saved me from pushing work I would have quietly regretted. That is most of what taste is anyway: small refusals, repeated until they become invisible.',
    ],
  },
]
